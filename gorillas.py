...
SUN_SHOCKED_SURF = makeSurfaceFromASCII(SUN_SHOCKED_ASCII, SUN_COLOR,
                                        SKY_COLOR)
STAR_SURF = makeSurfaceFromASCII(STAR_ASCII, DARK_RED_COLOR, BLACK_COLOR)

assert GOR_DOWN_SURF.get_size() == GOR_LEFT_SURF.get_size(
) == GOR_RIGHT_SURF.get_size()
"""Create the pygame.Surface objects from the ASCII strings."""

sunRect = pygame.Rect(SUN_X, SUN_Y, SUN_NORMAL_SURF.get_width(),
                      SUN_NORMAL_SURF.get_height())
...
def drawText(text, surfObj, x, y, fgcol, bgcol, pos='left'):
    ...
    textobj = GAME_FONT.render(
        text, 1, fgcol,
        bgcol)  # creates the text in memory (it's not on a surface yet).
...
def inputMode(prompt,
    ...
    while not done:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if event.key == K_ESCAPE:
                    ...
                    if len(inputText):
                        drawText(prompt + inputText + cursorShow, screenSurf,
                                 textrect.left, textrect.top, bgcol, bgcol,
                                 'left')
                        inputText = inputText[:-1]
                else:
                    if len(inputText) >= maxlen or (allowed is not None
                                                    and chr(event.key)
                                                    not in allowed):
                        continue
        ...
        textrect = drawText(prompt + cursorShow, screenSurf, x, y, fgcol,
                            bgcol, pos)
        drawText(prompt + inputText + cursorShow, screenSurf, textrect.left,
                 textrect.top, fgcol, bgcol, pos='left')
...
def makeCityScape():
    ...
    screenSurf = pygame.Surface((
        640,
        SCR_HIEGHT))  # first make the new surface the same size of the screen.
    screenSurf.fill(
        SKY_COLOR)  # fill in the surface with the background sky color
    ...
    buildingCoords = [
    ]  # a list of (left, top) coords of each building, left to right

    x = 2  # x refers to the top left corner of the current building being drawn

    while x < 640 - heightInc:
        ...
        buildingColor = BUILDING_COLORS[random.randint(
            0,
            len(BUILDING_COLORS) - 1)]

        # Draw the building
        pygame.draw.rect(screenSurf, buildingColor,
                         (x + 1, bottomLine -
                          (buildHeight + 1), buildWidth - 1, buildHeight - 1))

        buildingCoords.append((x, bottomLine - buildHeight))

        # Draw the windows
        for winx in range(3, buildWidth - windowSpacingX + windowWidth,
                          windowSpacingX):
            for winy in range(3, buildHeight - windowSpacingY, windowSpacingY):
                ...
                pygame.draw.rect(screenSurf, winColor,
                                 (x + 1 + winx,
                                  (bottomLine - buildHeight) + 1 + winy,
...
def placeGorillas(buildCoords):
    ...
    gorPos = [
    ]  # item 0 is for (left, top) of player one, item 1 is for player two.
    ...
    for i in range(0, 2):  # place first and then second player
        ...
        if i == 0:
            ...
            buildNum = random.randint(
                len(buildCoords) - 3,
                len(buildCoords) - 2)

        buildWidth = buildCoords[buildNum + 1][0] - buildCoords[buildNum][0]
        gorPos.append((buildCoords[buildNum][0] + int(buildWidth / 2) - xAdj,
                       buildCoords[buildNum][1] - yAdj - 1))
...
def showStartScreen(screenSurf):
    ...
    while not checkForKeyPress():
        ...
        drawText('P  y  t  h  o  n     G  O  R  I  L  L  A  S',
                 screenSurf,
                 640 / 2,
                 50,
                 WHITE_COLOR,
                 BLACK_COLOR,
                 pos='center')
        drawText('Your mission is to hit your opponent with the exploding',
                 screenSurf,
                 640 / 2,
                 110,
                 GRAY_COLOR,
                 BLACK_COLOR,
                 pos='center')
        drawText('banana by varying the angle and power of your throw, taking',
                 screenSurf,
                 640 / 2,
                 130,
                 GRAY_COLOR,
                 BLACK_COLOR,
                 pos='center')
        drawText('into account wind speed, gravity, and the city skyline.',
                 screenSurf,
                 640 / 2,
                 150,
                 GRAY_COLOR,
        ...
        drawText(
            ...
            BLACK_COLOR,
            pos='center')
        drawText('of the playing field, its length relative to its strength.',
                 screenSurf,
                 640 / 2,
                 190,
                 GRAY_COLOR,
...
def drawStars(screenSurf, vertAdj, horAdj):
    ...
    for i in range(16):
        ...
        screenSurf.blit(STAR_SURF,
                        (2 +
                         (((3 - vertAdj) + i * 4) * STAR_SURF.get_width()), 3))
        # draw bottom row of stars
        screenSurf.blit(STAR_SURF,
                        (2 + ((vertAdj + i * 4) * STAR_SURF.get_width()),
                         SCR_HEIGHT - 7 - STAR_SURF.get_height()))

    for i in range(4):
        # draw left column of stars going down
        screenSurf.blit(STAR_SURF,
                        (5, 6 + STAR_SURF.get_height() + (horAdj + i * 84)))
        # draw right column of stars going up
        screenSurf.blit(STAR_SURF, (640 - 5 - STAR_SURF.get_width(),
                                    (SCR_HEIGHT - (6 + STAR_SURF.get_height() +
                                                   (horAdj + i * 84)))))
...
def showSettingsScreen(screenSurf):
    ...
    while p1name is None:
        p1name = inputMode("Name of Player 1 (Default = 'Player 1'):  ",
                           screenSurf,
                           640 / 2 - 146,
                           50,
                           GRAY_COLOR,
    ...
    while p2name is None:
        p2name = inputMode("Name of Player 2 (Default = 'Player 2'):  ",
                           screenSurf,
                           640 / 2 - 146,
                           80,
                           GRAY_COLOR,
    ...
    while points is None:
        points = inputMode("Play to how many total points (Default = 3)?  ",
                           screenSurf,
                           640 / 2 - 155,
                           110,
                           GRAY_COLOR,
    ...
    while gravity is None:
        gravity = inputMode("Gravity in Meters/Sec (Earth = 9.8)?  ",
                            screenSurf,
                            640 / 2 - 150,
                            140,
                            GRAY_COLOR,
...
def showIntroScreen(screenSurf, p1name, p2name):
    ...
    drawText('STARRING:',
             ...
             SKY_COLOR,
             pos='center')
    drawText('%s AND %s' % (p1name, p2name),
             screenSurf,
             640 / 2,
             115,
             WHITE_COLOR,
...
def getShot(screenSurf, p1name, p2name, playerNum):
    ...
    drawText('Velocity:   ' + str(angle), screenSurf, x, 2, SKY_COLOR,
             SKY_COLOR)
    pygame.display.update()
...
def displayScore(screenSurf, oneScore, twoScore)
    ...
    drawText(str(oneScore) + '>Score<' + str(twoScore),
             screenSurf,
             270,
             310,
             WHITE_COLOR,
...
def plotShot(screenSurf, skylineSurf, angle, velocity, playerNum, wind,
             gravity, gor1, gor2):
    ...
    starty -= getBananaRect(0, 0,
                            bananaShape).height + BAN_UP_SURF.get_size()[1]
    ...
    while not impact and bananaInPlay:
        ...
        if x >= 640 - 10 or x <= 3 or y >= SCR_HEIGHT:
            bananaInPlay = False
        ...
        if bananaInPlay and y > 0:
            ...
            if bananaRect.colliderect(gor1rect):
                ...
                doExplosion(screenSurf,
                            skylineSurf,
                            bananaRect.centerx,
                            bananaRect.centery,
                            explosionSize=int(GOR_EXPLOSION_SIZE * 2 / 3),
                            speed=0.005)
                doExplosion(screenSurf,
                            skylineSurf,
                            bananaRect.centerx,
                            bananaRect.centery,
                            explosionSize=GOR_EXPLOSION_SIZE,
                            speed=0.005)
                drawSun(screenSurf)
                return 'gorilla1'
                ...
                drawSun(screenSurf)
                return 'gorilla2'
            elif collideWithNonColor(srcPixArray, screenSurf, bananaRect,
                                     SKY_COLOR):
                # banana has hit a building
                doExplosion(screenSurf, skylineSurf, bananaRect.centerx,
                            bananaRect.centery)
                screenSurf.fill(SKY_COLOR, bananaRect)  # erase banana
                drawSun(screenSurf)
                return 'building'
...
daf victoryDance(screenSurf, x, y):
...
def drawWind(screenSurf, wind):
    ...
    if wind != 0:
        wind *= 3
        pygame.draw.line(screenSurf, EXPLOSION_COLOR,
                         (int(640 / 2), SCR_HEIGHT - 5),
                         (int(640 / 2) + wind, SCR_HEIGHT - 5))
        # draw the arrow end
        if wind > 0: arrowDir = -2
        else: arrowDir = 2
        pygame.draw.line(screenSurf, EXPLOSION_COLOR,
                         (int(640 / 2) + wind, SCR_HEIGHT - 5),
                         (int(640 / 2) + wind + arrowDir, SCR_HEIGHT - 5 - 2))
        pygame.draw.line(screenSurf, EXPLOSION_COLOR,
                         (int(640 / 2) + wind, SCR_HEIGHT - 5),
                         (int(640 / 2) + wind + arrowDir, SCR_HEIGHT - 5 + 2))


def doExplosion(screenSurf,
                skylineSurf,
                x,
                y,
                explosionSize=BUILD_EXPLOSION_SIZE,
                speed=0.05):
...
def main():
    winSurface = pygame.display.set_mode((640, SCR_HEIGHT), 0, 32)
    ...
    ##winSurface = pygame.display.set_mode((640, SCR_HEIGHT), pygame.FULLSCREEN, 32)
    ...
    while True:
        # start a new game
        p1name, p2name, winPoints, gravity, nextScreen = showSettingsScreen(
            winSurface)
        if nextScreen == 'v':
        ...
        while p1score < winPoints and p2score < winPoints:
            if newRound:
                ...
                )  # Note that the city skyline goes on skylineSurf, not winSurface.
            ...
            result = plotShot(winSurface, skylineSurf, angle, velocity, turn,
                              wind, 9.8, gorPos[0], gorPos[1])
